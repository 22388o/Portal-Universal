...
class SendBTCService: ISendAssetService {
    ...
    var spendable: Decimal {
        sendAdapter.availableBalance(feeRate: feeRate.value, address: receiverAddress.value, pluginData: [:])
    }
    
    var fee: Decimal {
        sendAdapter.fee(amount: amount.value > 0 ? amount.value : 0.0001, feeRate: feeRate.value, address: receiverAddress.value, pluginData: [:])
    }
    ...
    func validateAddress() throws {
        try sendAdapter.validate(address: receiverAddress.value, pluginData: [:])
    }
